-- КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ RLS ПОЛИТИК
-- Выполни это в Supabase SQL Editor ПРЯМО СЕЙЧАС
...
-- 5. Тестовый запрос для проверки прав
INSERT INTO players (nickname, password, faction, gender) 
VALUES ('test_user_' || extract(epoch from now())::text, '123', 'workers', 'male')
ON CONFLICT (nickname) DO NOTHING
RETURNING id, nickname, 'REGISTRATION_TEST_OK' as status;
